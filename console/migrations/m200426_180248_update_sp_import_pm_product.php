...
class m200426_180248_update_sp_import_pm_product extends Migration
{
    public function safeUp()
    {
        $this->execute("DROP PROCEDURE SP_Import_PM_Product");
        $this->execute("CREATE PROCEDURE [dbo].[SP_Import_PM_Product]
AS
    SET NOCOUNT ON;
...
INSERT #ERRORS(USERNAME, CAUSE)
SELECT tpp.Username, 'USERNAME INEXISTENTE'
FROM TEMP_PM_PRODUCT tpp
WHERE NOT EXISTS(SELECT 1 FROM [user] u WHERE u.Username = tpp.Username)
GROUP BY tpp.Username
ORDER BY tpp.Username
...
ORDER BY tpp.TradeProductId
    ...
    ELSE
        BEGIN
            ...
            SELECT tpp.TradeProductId, tpp.GmidId, u.UserId
            INTO #associations
            FROM TEMP_PM_PRODUCT tpp
                     INNER JOIN [user] u ON tpp.Username = u.Username;
