...
class m150824_155033_store_procedures extends Migration {

    public function up() 
    {    
    $this->execute("CREATE PROCEDURE [dbo].[SP_ImportSales]
...
AS
	...
	SELECT F3,GMID,[Calendar month],Actual,Total,@ActualCampaignId
	FROM TEMP_SALE
...
");
    ...
    $this->execute("CREATE PROCEDURE [dbo].[SP_ImportCustomer]
AS
  SET NOCOUNT ON;
...
-- CLIENTS PRODUCTS
  ...
  END;
    DELETE FROM TEMP_CUSTOMER;");
       ...
       $this->execute("CREATE PROCEDURE [dbo].[SP_ImportProducts]
AS
	SET NOCOUNT ON;
...
WHERE fore.ClientProductId IS NULL 
 ...
 END;
	...
	DROP TABLE  #ERRORS
");
    $this->execute("CREATE PROC [dbo].[CreateConfigCampaign] 
    AS
    ...
    public function down() {
        echo "m150824_155033_store_procedures cannot be reverted.\n";
