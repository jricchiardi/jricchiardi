...
<?php endforeach; ?>
    ...
    cascadeFrom: "filterdashboard-performancecenterid",
            ...
            $("#filterdashboard-rsmid").kendoDropDownList({
    optionLabel: "<?=Yii::t('app','Select') ?>",
            filter: "startswith",
            ...
            $("#filterdashboard-dsmid").kendoDropDownList({
    cascadeFrom: "filterdashboard-rsmid",
    ...
    cascadeFrom: "filterdashboard-dsmid",
    ...
    cascadeFrom: "filterdashboard-sellerid",
            ...
            dataSource: {
            data: <?= yii\helpers\Json::encode(\Yii::$app->db->createCommand('SELECT [client].[ClientId], [client].[Description], [client_seller].[SellerId] AS [SellerId] FROM [client] LEFT JOIN [client_seller] ON [client].[ClientId] = [client_seller].[ClientId] WHERE [GroupId] IS NULL ORDER BY [client].[Description]')
            ->queryAll()) ?>
            }
    });
