...
<script>
    ...
    $(document).ready(function () {
      ...
      $("#filterdashboard-dsmid").kendoDropDownList({
            ...
            dataValueField: "UserId",
            dataSource: {
            data: <?= yii\helpers\Json::encode(common\models\User::find()->select(['UserId','user.ParentId AS RsmId','user.Fullname'])->joinWith('itemNames')->where(['[auth_item].name' => common\models\AuthItem::ROLE_DSM,'ParentId'=>  Yii::$app->user->identity->UserId])->orderBy('Fullname ASC')->asArray()->all()) ?>
            }
        });
