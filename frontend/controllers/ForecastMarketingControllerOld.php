...
use common\models\ForecastMarketingOld;
 ...
 * Site controller
 */
class ForecastMarketingControllerOld extends CustomController
{
    ...
    public function actionExport()
    {
        ...
        $tradeProductIds = implode(',', $this->getTradeProductIds($UserId));
        $clientIds = implode(',', $this->getClientIds($UserId));

        $gmids = implode(',', $this->getGmids($UserId));
        $andGmids = empty($gmids) ? "" : "AND ([GmidId] IN ($gmids))";
...
FROM [SaleWithForecastMarketing] [swfm]
         INNER JOIN client_marketing cm on swfm.ClientMarketingId = cm.ClientMarketingId
         INNER JOIN country c on cm.CountryId = c.CountryId
WHERE (([CampaignId] = $CampaignId)
           AND ([IsForecastable] = 1)
           AND ([TradeProductId] IN ($tradeProductIds))
           $andGmids
           AND (swfm.[ClientMarketingId] IN ($clientIds))
           AND ([SellerId] = $UserId))
...
ORDER BY [ForecastDescription]
    ...
    public function actionGetClients($CountryId, $TradeProductId = null, $GmidId = null)
    {
        ...
        $userId = Yii::$app->user->identity->UserId;
        $CampaignId = Campaign::getActualCampaign()->CampaignId;
        $tradeProductIds = $this->getTradeProductIds($userId);
        $clientIds = $this->getClientIds($userId, $CountryId);

        $gmids = $this->getGmids($userId);
        ...
        $query->where([
            ...
            "IsForecastable" => "1",
            'TradeProductId' => $tradeProductIds,
            'ClientMarketingId' => $clientIds,
            'SellerId' => $userId,
        ]);
        $query->andWhere("CampaignSale = :CampaignId OR CampaignSale IS NULL", [":CampaignId" => $CampaignId]);

        if (!empty($gmids)) {
            $query->andWhere(["GmidId" => $gmids]);
        }
    ...
    public function actionGetProducts($CountryId)
    {
        ...
        $UserId = Yii::$app->user->identity->UserId;
        $tradeProductIds = implode(',', $this->getTradeProductIds($UserId));
        $clientIds = implode(',', $this->getClientIds($UserId, $CountryId));

        $gmids = implode(',', $this->getGmids($UserId));
        $andGmids = empty($gmids) ? "" : "AND g.GmidId IN ($gmids)";
...
WHERE cp.IsForecastable = 1
  AND tp.TradeProductId IN ($tradeProductIds)
  AND cp.ClientMarketingId IN ($clientIds)
...
ORDER BY Description
    ...
    public function actionSave()
    {
        ...
        foreach ($modelsPost as $item) {
            $model = ForecastMarketingOld::findOne([
    ...
    public function actionExportReportConsolid()
    {
        ...
        $sql = "
SELECT DISTINCT *
FROM ExportConsolidMarketing
INNER JOIN pm_client pc ON [Product Manager] = pc.UserId AND Cliente = pc.ClientId
...
//        OPTION (FORCE ORDER)
    ...
    private function getTradeProductIds($userId)
    {
        $connection = Yii::$app->db;

        return $connection
            ->createCommand("SELECT DISTINCT TradeProductId FROM pm_product WHERE UserId = $userId")
            ->queryColumn();
    }

    private function getGmids($userId)
    {
        $connection = Yii::$app->db;

        return $connection
            ->createCommand("SELECT DISTINCT GmidId FROM pm_product WHERE UserId = $userId")
            ->queryColumn();
    }

    private function getClientIds($userId, $countryId = null)
    {
        $connection = Yii::$app->db;
...
SELECT DISTINCT ClientId
FROM pm_client
         INNER JOIN client_marketing cm on pm_client.ClientId = cm.ClientMarketingId
WHERE UserId = $userId
  AND cm.CountryId = $countryId
            ")->queryColumn();
    }
}
