...
ORDER BY [ForecastDescription]
    ...
    public function actionGetClients($CountryId, $ClientMarketingProductId = null)
    {
        ...
        if (!empty($ClientMarketingProductId)) {
            $query->andWhere("swfm.ClientMarketingProductid = $ClientMarketingProductId");
        }
...
SELECT [cp].[ClientMarketingProductId],
       tp.Description
...
GROUP BY ClientMarketingProductId, tp.Description
UNION
SELECT [cp].[ClientMarketingProductId],
       g.Description
FROM [client_marketing_product] [cp]
...
WHERE (((cp.IsForecastable = 1) AND (pp.UserId = $UserId)) AND (cm.CountryId = $CountryId))
  AND (cpm.UserId = $UserId)
  AND vc.ValueCenterId <> 10111
GROUP BY ClientMarketingProductId, g.Description
            ")->queryAll();
    }
