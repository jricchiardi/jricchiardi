...
ORDER BY Pais, DSM, PerformanceCenter, TradeProduct, Gmid
        ...
        $finish = ($monthEnableFrom - 1) + Yii::$app->utilcomponents->getAmountQuarter($monthEnableFrom - 1);
    ...
    public function actionExportReportConsolid()
    {
        ...
        $sql = "
SELECT *
FROM (SELECT            $campaign->CampaignId  AS CampaignId,
      ...
      FROM pm_product
               ...
               INNER JOIN pm_dsm ON pm_dsm.PmId = pm_product.UserId
               INNER JOIN [user] pm ON pm.UserId = pm_dsm.PmId
               INNER JOIN [user] u on u.UserId = pm_dsm.DsmId
               INNER JOIN (SELECT dsm.UserId AS DsmId, cm.CountryId AS DsmCountryId
                           FROM [user] dsm
                                    INNER JOIN auth_assignment aa on dsm.UserId = aa.user_id
      ...
      WHERE (f.CampaignId IS NULL OR f.CampaignId = $campaign->CampaignId)
        AND pc.ValueCenterId <> 10111
      UNION
      SELECT            $campaign->CampaignId  AS CampaignId,
                        country.Description    AS Country,
                        vc.Description         AS ValueCenter,
                        pm.UserId              AS ProductManagerId,
                        pm.Fullname            AS ProductManager,
      ...
      WHERE (f.CampaignId IS NULL
          OR f.CampaignId = $campaign->CampaignId)
        AND pc.ValueCenterId = 10111) AS result
         UNPIVOT
         (
...
WHERE Volume <> 0
  AND CampaignId = $campaign->CampaignId
        ...
        foreach ($consolidates as $key => $item) {
            ...
            $numberMonth = Yii::$app->utilcomponents->getNumberMonth($item["Month"]);
            $numberQuarter = Yii::$app->utilcomponents->_getNumberQuarter($numberMonth);
            ...
            $objSheet->SetCellValue(chr($charCol++) . $row, $item["GmidId"]);
            $objSheet->SetCellValue(chr($charCol++) . $row, $item["Gmid"]);
            $objSheet->SetCellValue(chr($charCol++) . $row, $numberMonth);
            $objSheet->SetCellValue(chr($charCol++) . $row, $numberQuarter);
    ...
    private function getNoSeedProductForecasts($pmId, $campaignId, $countryId, $tradeProductId, $gmidId)
    {
        $sql = $this->getNoSeedSql($pmId, $campaignId, $countryId, "
            AND tp.TradeProductId = $tradeProductId
            AND g.GmidId = $gmidId
        ");
    ...
    {
        $seedSql = $this->getSeedSql($pmId, $campaignId, $countryId, "
            AND tp.TradeProductId = $tradeProductId
        ");

        $sql = "$seedSql
        GROUP BY ForecastMarketingId,
         DsmId,
         Dsm,
         CampaignId,
         TradeProductId,
