...
-- BEGIN
    ...
    DELETE FROM one_gmid_per_tradeproduct WHERE 1=1;
    INSERT INTO one_gmid_per_tradeproduct
    select Gmid.TradeProductId, (SELECT TOP 1 GmidAux.GmidId FROM gmid AS GmidAux WHERE GmidAux.TradeProductId = Gmid.TradeProductId AND GmidAux.IsActive = 1 and GmidAux.IsForecastable = 1) AS GmidId
    from gmid Gmid
    where TradeProductId not in (select TradeProductId from one_gmid_per_tradeproduct)
    group by Gmid.TradeProductId;
    ...
    [user] AS Dsm
        ...
        FROM
            forecast AS Forecast
                ...
                INNER JOIN one_gmid_per_tradeproduct AS Gmid ON
                    Gmid.TradeProductId = ClientProduct.TradeProductId
        GROUP BY
            ClientProduct.ClientId,
            Gmid.GmidId,
